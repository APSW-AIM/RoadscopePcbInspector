...
void SettingsDialog::showEvent(QShowEvent* pShowEvent)
{
    fillPortsInfo();
...
void SettingsDialog::fillPortsInfo()
{
    ...
    for (const QSerialPortInfo& info : infos)
    {
        ...
        list << info.portName() << (!description.isEmpty() ? description : blankString)
             << (!manufacturer.isEmpty() ? manufacturer : blankString)
             << (!serialNumber.isEmpty() ? serialNumber : blankString) << info.systemLocation()
        ...
        if (info.portName().compare(m_currentSettings.name) == 0)
        {
            selectedPortIndex = loopPortIndex;
        }
